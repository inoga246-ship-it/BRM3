...
function renderGraphScale(targetDistance) {
  ...
  // 中間ポイントは常に「viewStart + 5km」地点を表示（5km手前が存在する場合）
  const midDist = viewStart + 5;
  if (midDist < viewEnd - 0.5) {
    const midPct = ((midDist - viewStart) / viewSpan) * 100;
    const midEle = getGpxElevationAtDistance(midDist);
    const midEleStr = midEle !== null ? midEle + "m" : "";
    const midHtml = midEleStr
      ? `<span class="mid-ele">${midEleStr}</span><br>${midDist.toFixed(1)}km`
      : `${midDist.toFixed(1)}km`;
    createScalePoint(midPct, midHtml, "mid-type", "2px", null);
  }
